  ...
  uic.ReportView = class ReportView {
    ...
    update() {
      ...
      keys.forEach((key, index) => {
        ...
        let resize = (e) => {
          dx = e.clientX - sx;
          $resizerGuide.css('left', offx + dx);
        };
        let stopResize = (e) => {
          dx = e.clientX - sx;
          if (typeof colOptions.width !== 'number')
            colOptions.width = 200;
          const finalWidth = Math.max(10, colOptions.width + dx);
          colOptions.width = finalWidth;
          window.removeEventListener('mousemove', resize);
          window.removeEventListener('mouseup', stopResize);
          document.body.classList.remove('uic-report__resizing-column');
          this.update();
        };
        $resizer.on('mousedown', (e) => {
          sx = e.clientX;
          offx = sx - $list[0].getBoundingClientRect().left;
          window.addEventListener('mousemove', resize);
          window.addEventListener('mouseup', stopResize);
          e.preventDefault();
          $resizerGuide.css('left', offx).show();
          document.body.classList.add('uic-report__resizing-column');
        });
        $item.append($resizer);
        $li.append($item);
      });
